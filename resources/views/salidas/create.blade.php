                  ...
                  <div class="row">
                    <div class="col-xl-12 ">{{-- offset-md-1 --}}
                      <div class="d-flex justify-content-between mb-4">
                        <h4 class="card-title mb-0">Vale de Consumo </h4>
                      </div>
                      ...
                      @if(session()->has('info'))
                        ...
                        <a href="{{ route('salidas.index') }}" class="btn btn-success">Regresar</a>

                      @else
                        <form class="forms-sample" id="ingresosForm2" action="{{ route('salidas.store') }}" method="post">
                          {!! csrf_field() !!}
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content"> 
        <form  id="f_proveedor" name="f_proveedor" method="post" action="{{ route('proy.store') }}" class="formarchivo" >
            {!! csrf_field() !!}
        <div class="modal-header">
          <h5 class="modal-title text-dark" id="exampleModalLabel">Registrar Vehículo u Oficina</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span> 
          </button>
        </div>
        <div class="modal-body pt-0 form-act">
          <input type="hidden" name="tc_id" name="tc_id" value="{{ 0}}">{{-- $tc_id --}}
                      ...
                      <div class="form-group row">
                        <label for="nom_proy" class="col-sm-4 col-form-label d-block">Nombre <span class="text-danger">*</span></label>
                        <div class="col-sm-8">
                        <input type="text" required="" autocomplete="off" class="form-control text-uppercase" name="nom_proy" placeholder="" value="{{ old('nom_proy') }}" />
                        </div>
                      </div>

                      <div class="form-group row">
                        <label for="direccion" class="col-sm-4 col-form-label d-block">Descripción </label>
                        <div class="col-sm-8">
                      <input type="text" class="form-control text-uppercase" name="direccion" placeholder="Descripción" value="" />{{-- {{ old('direccion') }} --}}
                        </div>
                      </div>
                      ...
                      <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                         <input type="hidden" required="" readonly="" class="form-control text-uppercase" name="cta_cte" id="cta_cte" placeholder="RUC" value="2050111111" />
                        </div>
                      </div>

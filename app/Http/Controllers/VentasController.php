...
class VentasController extends Controller
{
    ...
    public function __construct(ProductRepositorie $prodRepo)
    {
        ...
        $this->formas = ['EFECTIVO','CHEQUE',4=>'TARJETA DE CRÉDITO',6=>'TRANSFERENCIA'];
    }
    ...
    public function a4(Comprobante $comprobante)
    {
        $this->actualizarSesion();
        //VERIFICA SI TIENE EL PERMISO
        if (!isset(session("permisosTotales")["salidas"]["permisos"]["editar"])) {
        ...
        $data = $this->documento($comprobante);
        $pdf = PDF::loadView('ventas.a4', $data)->setPaper('letter', 'portrait');//->setOptions(['dpi' => 650 ])->setPaper($dimension);;
        return $pdf->stream();
    }
    ...
    function documento(Comprobante $comprobante){
        ...
        $detalle = Comprobante_Det::select("cod_articulo", "precio", "cantidad","articulos.nombre","articulos.stock_min","unidad_medida.cod_umedida")
            ->join("articulos","articulos.cod_artic","=","comprobantes_det.cod_articulo")
            ->join("unidad_medida","articulos.cod_umedida","=","unidad_medida.id")
            ->where("cod_comprobante","=",$comprobante->id)
            ->get();
        $comprobante->tot_reg=$detalle->count();
        ...
        $titulo = "{$tipo} {$titulo_nro}";
        $empresa = new stdClass();
        $empresa->nombre = "FARMA SALUD Y VIDA S.A.C";
        $empresa->ruc = "20606533943";
        $empresa->direccion = "Lima, Callelos sauces 235";
        $empresa->telefono = "925 316 984";
        $empresa->email = "farma@gmail.com";
        $fecha = $comprobante->created_at->format('d/m/Y');
        $hora = $comprobante->created_at->format('H:i');
        $cliente = new stdClass();
        $cliente->nombre = $comprobante->razon_social;
        $cliente->nro = $comprobante->cod_cliente;
        $cliente->direccion = $comprobante->direccion;

        $monto = $comprobante->importe - $comprobante->igv_total;

        $subtotal = number_format($comprobante->importe,2);
        $igv = number_format($comprobante->igv_total,2);
        $total = number_format($comprobante->total,2);
        $descuento = number_format($comprobante->descuento_valor,2);
        $monto = number_format($monto,2);
        $condicion = $this->formas[$comprobante->forma_pago]??"";
        $vendedor = $comprobante->cod_usuario;
        $total0 = $comprobante->total;
        $texto = "";
        $codigo = DNS2D::getBarcodeHTML('$empresa->ruc'.' | '.'$venta->tipoComprobante->idtipo_comprobante' .' | '.'$venta->serie_comprobante'.' | '.'$venta->num_comprobante'.' | '.'$venta->igv'.' | '.'$venta->total'.' | '.'now()->parse($venta->creado)->format("Y-m-d")'.' | '.'$venta->cliente->tipoDocumento->idtipo_documento'.' | '.'$venta->cliente->num_documento', 'QRCODE',4,4);
        $hash="hash: 123fswefghb===xvreee";
        $data = compact("tipo","titulo","serie","nro","fecha","hora",
            "empresa","cliente","detalle",
            "subtotal","igv","total","monto","descuento","total0",
            "condicion","vendedor",
            "texto","codigo","hash");
        return $data;
    }

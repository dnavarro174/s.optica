...
class GuiaDController extends Controller
{   
    private $_prodRepo;
    ...
    public function index(Request $request)
    {
        ...
        if($text_fecha || $search){
            ...
            if($text_fecha and !$search){
                ...
                $guiaD_datos = Salida::join('proyectos as p', 'p.id','=','movimientos.proyectos_id')
            ...
            $guiaD_datos = Cache::rememberForever($key, function() use ($pag, $tpo_doc){
                return Salida::join('proyectos as p', 'p.id','=','movimientos.proyectos_id')
                    ->select('movimientos.id','movimientos.ano_doc','movimientos.mes_doc',
                        'movimientos.nro_ref','movimientos.nro_doc','movimientos.responsable',
                        'movimientos.proyectos_id','movimientos.created_at',
                        'movimientos.doc_tipo','movimientos.doc_estado'
                    )
        ...
        return view('guia_devolucion.index', compact('guiaD_datos','fechas', 'permisos'));
    }

    public function stock(Request $request){

        //return "Stock cero: id=".$request->id.' cant: '.$request->cant.' proy: '.$request->proy;
        $cod_artic = $request->id;
        $cant      = $request->cant;
        $proy      = $request->proy;
        
        // para verificar, el producto debe tener salida para poder devolver
        $cantProductosxProyecto = Movimientos::
                select('cant_mov')
                ->where('tpo_doc','04')
                ->where('cod_empresa', session('cod_empresa'))
                ->where('cod_almacen', session('cod_almacen'))
                ->where('cod_artic', $cod_artic)
                ->where('proyectos_id', $proy)
                ->sum('cant_mov');

        $existe_Devolucion = Movimientos::
                select('cant_mov')
                ->where('tpo_doc','02')
                ->where('cod_empresa', session('cod_empresa'))
                ->where('cod_almacen', session('cod_almacen'))
        ...
        $a = ($cantProductosxProyecto) ? $cantProductosxProyecto : 0;
        $b = ($existe_Devolucion) ? $existe_Devolucion : 0;

        $c = $a - $b;
        //return "$a - $b - $c ";

        /*select sum(cant_mov) from movimientos where tpo_doc='02' or tpo_doc='04'  and cod_empresa=1 and cod_almacen=1 and cod_artic=2 and proyectos_id=10 */

        $stock_arr = array();
        if($c){
            $a_a = $c;
            $a   = number_format($a_a,2);
            if($a_a > 0){
                if($a_a >= $cant){
                    $stock_arr = array('stock'=>'D','stock_bd'=>$a);
                }else{
                    $stock_arr = array('stock'=>'C','stock_bd'=>$a);
                }
            }else{
                $stock_arr = array('stock'=>'B','stock_bd'=>$a);
            }
        }else{
            $stock_arr = array('stock'=>'A','stock_bd'=>0);// no tiene stock
        }
    ...
    public function create()
    {
        ...
        return view('guia_devolucion.create',compact('monedas', 'numbers'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $flag_pcosto  = "N";
        $orden_compra   = null;
        $flag_anulado   = "N"; //ruc_ctacte = null
        $flag_facturado = "N";
        $tipo           = 0;
        $serie_d        = '000';
    ...
    { 
        ...
        if(!$datos){
            return redirect()->route('guia_devolucion.index');
        }
        ...
        return view('guia_devolucion.edit',compact('datos', 'proyectos', 'tipos', 'empresa', 'items'));
    }
    ...
    {   
        ...
        foreach ($tipo_doc as $value) {
            ...
            // Tengo que quitar las cantidades aumentadas en la tb articuos
